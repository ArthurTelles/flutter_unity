...
namespace FlutterUnityIntegration.Editor
{
    public class Build : EditorWindow
    {
        ...
        private static readonly string APKPath = Path.Combine(
            ProjectPath,
            "Builds/" + Application.productName + ".apk"
        );

        private static readonly string AndroidExportPath = Path.GetFullPath(
            Path.Combine(ProjectPath, "../../android/unityLibrary")
        );
        private static readonly string WindowsExportPath = Path.GetFullPath(
            Path.Combine(ProjectPath, "../../windows/unityLibrary/data")
        );
        private static readonly string IOSExportPath = Path.GetFullPath(
            Path.Combine(ProjectPath, "../../ios/UnityLibrary")
        );
        private static readonly string WebExportPath = Path.GetFullPath(
            Path.Combine(ProjectPath, "../../web/UnityLibrary")
        );
        private static readonly string IOSExportPluginPath = Path.GetFullPath(
        ...
        public static void DoBuildAndroidLibraryDebug()
        {
            ...
            Copy(
                ...
                Path.Combine(AndroidExportPath, "src/main/res")
            );
        }
        ...
        private static void DoBuildAndroid(String buildPath, bool isPlugin, bool isReleaseBuild)
        {
            ...
            EditorUserBuildSettings.il2CppCodeGeneration = UnityEditor
                .Build
...
#unity-mobile-warning { position: absolute; left: 50%; top: 5%; transform: translate(-50%); background: white; padding: 10px; display: none }
        ...
        private static void ModifyAndroidGradle(bool isPlugin)
        {
            ...
            proguardText = proguardText.Replace(
                "-ignorewarnings",
        ...
        private static void Copy(string source, string destinationPath)
        {
            ...
            foreach (
                var dirPath in Directory.GetDirectories(source, "*", SearchOption.AllDirectories)
            )
    ...
    spec.vendored_frameworks = ""UnityFramework.framework""
        ...
        private static async void BuildUnityFrameworkArchive()
        {
            ...
            try
            {
                ...
                await $"archive -workspace {xcprojectName} -scheme {schemeName} -sdk iphoneos -archivePath {buildPath}/Release-iphoneos.xcarchive ENABLE_BITCODE=NO |xcpretty".Bash(
                    "xcodebuild"
                );

                Debug.Log("### Copying framework files");
                await $" -RL {iosArchiveFrameworkPath} {iosRunnerPath}/{frameworkNameWithExt}".Bash(
                    "cp"
                );
                await $" -RL {iosArchiveFrameworkPath}/{dysmNameWithExt} {iosRunnerPath}/{dysmNameWithExt}".Bash(
                    "cp"
                );
